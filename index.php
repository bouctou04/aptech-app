...
if(!empty($_SESSION['user']) AND $_SESSION['user'] > 0) {
    header("Location: templates/accueil.php?id=". $_SESSION['user']);
}
...
if(isset($_POST['submitted'])) {
    if(!empty($_POST['username']) AND !empty($_POST['password'])) {
        ...
        if($login->login($_POST['username'], $_POST['password']) === true) {
            ...
            header("Location: templates/accueil.php?id=". $login->getId());
        } else {
					...
					<div class="col-12 pt-4">
						<form class="form" method="POST">
							<div class="form-group">
								...
								<?php $form->input('text', 'username', 'mail', 'form-control', '"Nom d\'utilisateur"', $value); ?>
							</div>
							<div class="form-group">
								<?php $form->label('mdp', 'Mot de passe', 'font-weight-bold h6'); ?>
								<?php $form->input('password', 'password', 'mdp', 'form-control', '"Mot de passe"'); ?>
							</div>
							<div class="form-group">
